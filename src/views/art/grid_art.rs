...
use gtk::{gdk, gdk_pixbuf, gdk_pixbuf::Pixbuf, graphene, gsk};
...
impl GridArt {
    ...
    pub fn load_multiple_pixbuf(&self, pixbufs: Vec<Rc<gdk_pixbuf::Pixbuf>>) {
        ...
        let full_pixbuf = gdk_pixbuf::Pixbuf::new(
        ...
        for (pos, pixbuf) in pixbuf_array.iter().enumerate() {
            ...
            pixbuf
                .scale_simple(scaled_size, scaled_size, gdk_pixbuf::InterpType::Bilinear)
                .unwrap()
                .composite(
                    &full_pixbuf,
                    offset_x,
    ...
    pub fn load_pixbuf(&self, pixbuf: Vec<Rc<gdk_pixbuf::Pixbuf>>) {
        let imp = self.imp();
        if pixbuf.len() > 0 {
            let new_pixbuf = match pixbuf[0].scale_simple(
                imp.size.get(),
                imp.size.get(),
