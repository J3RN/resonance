...
use crate::model::{artist::Artist, genre::Genre};
...
impl GenericFlowboxChild {
    pub fn new(
        child_type: GenericChild,
        artist: Option<Rc<Artist>>,
        genre: Option<Rc<Genre>>,
    ) -> GenericFlowboxChild {
        let generic_flowbox_child: GenericFlowboxChild =
            glib::Object::builder::<GenericFlowboxChild>().build();
    ...
    pub fn construct(
        ...
        let bg = match child_type {
            GenericChild::Artist => {
                ...
                imp.album_count_label.set_label(&i18n_k(
                    "{number_of_albums} albums",
                    &[("number_of_albums", &format!("{}", artist.n_albums()))],
                ));
                imp.track_count_label.set_label(&i18n_k(
                    "{number_of_tracks} tracks",
                    &[("number_of_tracks", &format!("{}", artist.n_tracks()))],
                ));
            ...
            GenericChild::Genre => {
                ...
                imp.album_count_label.set_label(&i18n_k(
                    "{number_of_albums} albums",
                    &[("number_of_albums", &format!("{}", genre.n_albums()))],
                ));
                imp.track_count_label.set_label(&i18n_k(
                    "{number_of_tracks} tracks",
                    &[("number_of_tracks", &format!("{}", genre.n_tracks()))],
                ));
